...
class OledDisplay {
  ...
  writeText(oled) {
    ...
    if (this._hasPageRow) {
      textToDisplay = this._text.slice(
          (this._currentPage - 1) * this._maxRow,
          this._maxRow,
      );
      const x = 1;
      const y = this._oledHeight - this._font.height - 1;
      const pagination = this._currentPage + '/' + this._totalPage;

      oled.setCursor(x, y);
      oled.writeString(this._font, 1, pagination, 1, false, 2);
    } else {
      textToDisplay = this._text;
    }
    ...
    textToDisplay.forEach((item, index) => {
      ...
      oled.setCursor(x, y);
      oled.writeString(this._font, 1, item, 1, false, 2);
    });
